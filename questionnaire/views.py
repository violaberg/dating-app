...
def questionnaire_view(request):
    ...
    if request.method == "POST":
        ...
        for question_id, field_name in question_to_field_mapping.items():
            ...
            for user_response in user_responses:
                if (
                    user_response.question.question_type == "single"
                    and user_response.answer
                ):
                    answer_value = user_response.answer.answer_text
                    setattr(profile, field_name, answer_value)

                elif (
                    user_response.question.question_type == "multiple"
                    and user_response.answer
                ):
                    answer_value_list = []
                    if user_response.answer:
                        answer_value_list.append(
                            user_response.answer.answer_text
                        )
    ...
    return render(
        ...
        {"questions": questions, "success_message": success_message},
    )
